...
final class PerformanceScreen extends ScrollView{
    ...
    private void show(int tab){removeAllViews();LinearLayout root=Ui.col(a);root.setPadding(24,18,24,22);addView(root);root.addView(Ui.text(a,"PRECISION PERFORMANCE",11,Ui.RED,true));root.addView(Ui.text(a,"Every heartbeat of the truck.",32,Color.WHITE,true));root.addView(Ui.text(a,"Live OBDLink MX+ data and a focused GPS acceleration timer.",14,Ui.SUB_TEXT,false));root.addView(Ui.tabs(a,new String[]{"Live Data","0–60","Gauges","History"},tab,this::show),new LinearLayout.LayoutParams(-1,76));
