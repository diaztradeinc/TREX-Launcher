...
public class MainActivity extends Activity {
    ...
    @Override public void onCreate(Bundle state){super.onCreate(state);Ui.applyTheme(this);getWindow().setStatusBarColor(Ui.BG);getWindow().setNavigationBarColor(Ui.BG);requestCorePermissions();if(!getSharedPreferences("trx",0).getBoolean("setup_done",false)){setContentView(new SplashScreen(this));return;}buildShell(state);}
    ...
    private View header(){
        LinearLayout h=Ui.row(this);h.setPadding(Ui.dp(this,28),0,Ui.dp(this,18),0);h.setBackground(Ui.bg(Ui.HEADER_BG,Ui.HEADER_STROKE,0,this));
    ...
    private View bottomNav(){
        LinearLayout bar=Ui.row(this);bar.setPadding(Ui.dp(this,10),Ui.dp(this,7),Ui.dp(this,10),Ui.dp(this,7));bar.setBackgroundColor(Ui.NAV_BAR_BG);
    ...
    public void showPage(int p){page=p;View oldSettings=stage.findViewWithTag("settings-screen");if(oldSettings!=null)stage.removeView(oldSettings);for(int i=0;i<pages.length;i++)pages[i].setVisibility(i==p?View.VISIBLE:View.GONE);
        ViewGroup bottom=(ViewGroup)((ViewGroup)stage.getParent()).getChildAt(2);for(int i=0;i<bottom.getChildCount();i++)bottom.getChildAt(i).setBackground(Ui.bg(i==p?Ui.RED:Ui.CARD2,i==p?0:Ui.CARD_STROKE,12,this));
    }
